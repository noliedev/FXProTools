...
if ( ( class_exists( 'LearnDash_Shortcodes_Section' ) ) && ( !class_exists( 'LearnDash_Shortcodes_Section_visitor' ) ) ) {
	class LearnDash_Shortcodes_Section_visitor extends LearnDash_Shortcodes_Section {
		...
		function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_title 		= 	__( 'Visitor', 'learndash' );
			$this->shortcodes_section_type			=	2;
			$this->shortcodes_section_description	=	sprintf( _x( 'This shortcode shows the content if the user is not enrolled in the %s. The shortcode can be used on <strong>any</strong> page or widget area.', 'placeholders: course', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'course' ) );
