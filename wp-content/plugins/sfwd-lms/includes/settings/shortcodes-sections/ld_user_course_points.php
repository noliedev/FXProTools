...
if ( ( class_exists( 'LearnDash_Shortcodes_Section' ) ) && ( !class_exists( 'LearnDash_Shortcodes_Section_ld_user_course_points' ) ) ) {
	class LearnDash_Shortcodes_Section_ld_user_course_points extends LearnDash_Shortcodes_Section {
		...
		function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_title 		= 	sprintf( _x( 'User %s Points', 'placeholder: Course', 'learndash' ), LearnDash_Custom_Label::get_label( 'course' ) );
			$this->shortcodes_section_type			=	1;
			$this->shortcodes_section_description	=	sprintf( _x( 'This shortcode shows the earned %s points for the user.', 'placeholders: course, course', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'course' ) );
