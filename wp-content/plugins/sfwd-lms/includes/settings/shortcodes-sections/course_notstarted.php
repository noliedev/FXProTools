...
if ( ( class_exists( 'LearnDash_Shortcodes_Section' ) ) && ( !class_exists( 'LearnDash_Shortcodes_Section_course_notstarted' ) ) ) {
	class LearnDash_Shortcodes_Section_course_notstarted extends LearnDash_Shortcodes_Section {
		...
		function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_title 		= 	sprintf( _x( '%s Not Started', 'placeholder: Course', 'learndash' ), LearnDash_Custom_Label::get_label( 'course' ) );
			$this->shortcodes_section_type			=	2;
			$this->shortcodes_section_description	=	sprintf( _x( 'This shortcode shows the content if the user has access to the %s but not yet started. The shortcode can be used on <strong>any</strong> page or widget area.', 'placeholders: course', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'course' ) );
