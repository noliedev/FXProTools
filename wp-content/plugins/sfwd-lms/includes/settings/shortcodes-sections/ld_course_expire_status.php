...
if ( ( class_exists( 'LearnDash_Shortcodes_Section' ) ) && ( !class_exists( 'LearnDash_Shortcodes_Section_ld_course_expire_status' ) ) ) {
	class LearnDash_Shortcodes_Section_ld_course_expire_status extends LearnDash_Shortcodes_Section {
		...
		function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_title 		= 	sprintf( _x( '%s Expire Status', 'placeholder: Course', 'learndash' ), LearnDash_Custom_Label::get_label( 'course' ) );
			$this->shortcodes_section_type			=	1;
			$this->shortcodes_section_description	=	sprintf( _x( 'This shortcode displays the user %s access expire date.', 'placeholders: course', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'course' ) );
		...
		function init_shortcodes_section_fields() {
			$this->shortcodes_option_fields = array(
				...
				'label_before' => array(
					...
					'help_text'		=>	__( 'The label prefix shown before the access expires', 'learndash' ),
					'value' 		=> 	'',
				),
				...
				'label_after' => array(
					...
					'help_text'		=>	__( 'The label prefix shown after access has expired', 'learndash' ),
					'value' 		=> 	'',
				)
			);
