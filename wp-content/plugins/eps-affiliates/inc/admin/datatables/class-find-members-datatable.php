...
*/
	...
	class Eps_find_members_data_table extends WP_List_Table {
	/**
  ...
  /**
	 ...
	 */
		function get_columns(){
		  $columns = array(
		  	...
		  	'member_network'	=> __( 'Member Network', 'affiliate-eps' ),
		  	'sponsor_network'	=> __( 'Sponsor Network', 'affiliate-eps' ),
	...
	*/
	 ...
	 * -----------------------------------------------------------------------------
	*/
	  public function column_member_network($item) {
				$value = sprintf( '<a title="%s" class="btn m-b-xs btn-sm  btn-addon" href="%s"><i class="fa fa-sitemap"></i></a>', __('Genealogy tree', 'affiliate-eps'), '#');
			  $value .= sprintf( '<a title="%s" class="btn m-b-xs btn-sm  btn-addon" href="%s"><i class="fa fa-slideshare"></i></a>', __('Referred Members', 'affiliate-eps'), '#');
			  $value .= sprintf( '<a title="%s" class="btn m-b-xs btn-sm  btn-addon" href="%s"><i class="fa fa-flask"></i></a>', __('Holding Tank', 'affiliate-eps'), '#');
				return $value;
		}
	/**
	 * -----------------------------------------------------------------------------
	...
	*/
	 ...
	 * -----------------------------------------------------------------------------
	*/
	  public function column_sponsor_network($item) {
			  $value = sprintf( '<a title="%s" class="btn m-b-xs btn-sm  btn-addon" href="%s"><i class="fa fa-sitemap"></i></a>', __('Genealogy tree', 'affiliate-eps'), '#');
			  $value .= sprintf( '<a title="%s" class="btn m-b-xs btn-sm  btn-addon" href="%s"><i class="fa fa-slideshare"></i></a>', __('Referred Members', 'affiliate-eps'), '#');
			  $value .= sprintf( '<a title="%s" class="btn m-b-xs btn-sm  btn-addon" href="%s"><i class="fa fa-flask"></i></a>', __('Holding Tank', 'affiliate-eps'), '#');
				return $value;
	...
	*/
    ...
    /**
		...
		*/
		function no_items() {
			_e( 'No affiliates found.', 'affiliate-eps' );
		}
 ...
 */
		public function process_bulk_action() {
			...
			if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-manage-members' ) && ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'manage-members-nonce' ) ) {
				return;
			}
	...
	*/
		function prepare_items() {
		  ...
		  $this->items 	= $this->affiliate_data($per_page, $current_page);
	 ...
	 *
	 * extra_tablenav is defult function and overwriting it
	 * 
	 * -----------------------------------------------------------------------------
	*/
	    ...
	    if ( $which == "top" ){
    		...
    		// do_action('eps_table_filter_reset');
   		}
		}
	/**
	 * -----------------------------------------------------------------------------
	 ...
	 * -----------------------------------------------------------------------------
	*/
		public function affiliate_data($per_page ='', $current_page ='') {
			...
			if (!empty($per_page) && !empty($current_page)) {
